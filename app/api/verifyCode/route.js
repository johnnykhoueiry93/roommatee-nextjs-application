...
async function setEmailVerified(isEmailVerified, emailAddress) {
    try {
      ...
      if(results.affectedRows > 0 ) {
        logger.info(`[${emailAddress}] - [${WS}] - The email is now verified.`);
      }
  
    } catch (err) {
        logger.error(`[${emailAddress}] - [${WS}] - Failed to update the isEmail verified to true: Root Cause: ` + err );
...
export async function POST(request) {
  try {
    ...
    logger.info(`[${emailAddress}] - [${WS}] - Backend verification started for email: ${emailAddress} and code: ${verificationCode}`);
    ...
    if(results.length > 0 ) {
        ...
        logger.info( `[${emailAddress}] - [${WS}] - The verification code: ${verificationCode} provided for the user: ${emailAddress} is invalid` );
        return NextResponse.json({ message: `Invalid Verification Code` }, { status: 401 });
    }

  } catch (err) {
    logger.error(`[${emailAddress}] - [${WS}] - Backend failure during the /api/changePassword webservice`, err);
    return NextResponse.json({ message: "Internal server error", error: err.message }, { status: 500 });
  }
}
